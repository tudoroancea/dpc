  ...
  def __init__(self, solver: Literal["fatrop", "ipopt"] = "ipopt", jit: bool = False):
    ...
    for i in range(Nf):
      ...
      if i > 0:
        ...
        cost_function += -q_delta_s * delta_s - q_delta_s_dot * delta_s_dot + q_n * n**2 + q_psi * psi**2 - q_v * v
    ...
    for i in range(Nf):
      ...
      if i > 0:
        # track constraints
        opti.subject_to((-1.75 <= x[i][1] - 0.5 * car_width * ca.cos(x[i][2]) + 0.5 * car_length * ca.sin(x[i][2])) <= 1.75)
        opti.subject_to((-1.75 <= x[i][1] + 0.5 * car_width * ca.cos(x[i][2]) + 0.5 * car_length * ca.sin(x[i][2])) <= 1.75)
        # velocity constraints
        opti.subject_to((0.0 <= x[i][3]) <= v_max)
    # terminal constraints
    opti.subject_to((-1.75 <= x[Nf][1] - 0.5 * car_width * ca.cos(x[Nf][2]) + 0.5 * car_length * ca.sin(x[Nf][2])) <= 1.75)
    opti.subject_to((-1.75 <= x[Nf][1] + 0.5 * car_width * ca.cos(x[Nf][2]) + 0.5 * car_length * ca.sin(x[Nf][2])) <= 1.75)
